...
- ✅ **Add-on Insights (Selesai 18 Apr 2026)**: Alur lengkap Insight → Artikel bekerja. `dispatchInsightGeneration` mengekstrak `<h1>` dari output AI sebagai judul post. `scrapeAndGenerateArticle` (untuk competitor & news) menggunakan Firecrawl REST API langsung. Editor membaca format `{ html: string }` dari DB secara benar.
...
| **17 Apr 2026** | Add-on `ai-insights` tidak ada plugin guard — siapapun bisa akses `/insights/*` meski plugin INACTIVE. | **Pelajaran**: Setiap Feature Add-on yang punya route mandiri WAJIB pasang guard di `layout.tsx`-nya: cek `tenantPlugins.status === "ACTIVE"` sebelum render. Jangan andalkan capability check saja. Lihat `docs/22-arsitektur-addon-standar.md` checklist P0. |
| **17 Apr 2026** | Menu Insights di sidebar tetap muncul meski `ai-insights` INACTIVE setelah toggle. | **2 Penyebab**: (1) `SidebarNav` hardcoded — tidak cek status addon. Solusi: pass `activeAddonIds` dari layout ke `SidebarNav` dan filter item `Insights` berdasarkan `"ai-insights" in activeAddonIds`. (2) `toggleAddonStatusAction` hanya `revalidatePath("/app/addons")` — layout tidak ikut di-revalidate. Solusi: tambah `revalidatePath("/app", "layout")` agar sidebar dapat data fresh. |
| **17 Apr 2026** | Semua perubahan di worktree sudah di-push ke `origin/main` tapi dev server masih pakai kode lama. | **Pelajaran**: Dev server jalan dari direktori main (`/Users/webane/sites/jalawarta/`), bukan dari worktree. Setelah push ke `origin/main`, wajib `git pull` di direktori main agar perubahan aktif. |
| **18 Apr 2026** | Sidebar CMS (`app.localhost`) kosong — hanya tampil Dashboard dan Tools. | **3 Penyebab Berantai**: (1) `app/app/layout.tsx` masih import `auth` dari `@/auth` (NextAuth yang sudah dipensiunkan) → `session = null` → role `undefined` → SidebarNav default ke `SUBSCRIBER`. Fix: ganti ke `getSession()` dari `@/lib/session`. (2) User `admin@jalawarta.local` di DB punya `role: "PLATFORM_ADMIN"` → `login.ts` short-circuit buat session `tenantId: null` → `hasCapability("PLATFORM_ADMIN", ...)` selalu `false` karena tidak ada di `ROLE_CAPABILITIES`. Fix: update DB `role` ke `TENANT_OWNER` agar melewati tenant lookup dan dapat `SUPER_ADMIN` dari `tenantMembers`. (3) Setelah role di DB diubah, user harus sign out + sign in agar session baru dibuat. |
| **18 Apr 2026** | Sign out redirect ke `/login` → 404 pada request pertama, baru muncul setelah di-refresh. | **Penyebab**: `logout()` server action memanggil `redirect("/login")` — redirect dari Server Action kadang tidak trigger middleware rewrite `/login → /app-login` dengan benar pada first request. **Solusi**: Hapus `redirect()` dari server action `logout()`, pindahkan redirect ke client: `SignOutButton.tsx` handle dengan `window.location.href = "/login"` (pola yang sama dengan fix login redirect 15 Apr). |
| **18 Apr 2026** | "Buat Artikel" di `/insights/competitor` error 500: `client.scrapeUrl is not a function`. | **Penyebab**: Ada dua implementasi Firecrawl berbeda di codebase — `src/lib/scraper/firecrawl.ts` pakai SDK `@mendable/firecrawl-js` dengan `client.scrapeUrl()` yang sudah berubah API-nya di versi baru; sementara `src/lib/insight-providers/firecrawl.ts` pakai raw `fetch` ke REST API `/v1/scrape` (bekerja). **Solusi**: Rewrite `src/lib/scraper/firecrawl.ts` hapus penggunaan SDK, ganti ke `fetch` langsung seperti implementasi yang bekerja. Sekaligus tambah `try/catch` di `scrapeAndGenerateArticle` agar error dikembalikan sebagai `{ success: false, error }` bukan throw → 500. |
